...
function strReplace(){
  var myStr = 'lettersAndDashes';
  if (wordRandom.contains(wordRandom.letter));
  var newStr = myStr.replace(/_/g, "guess");
}
  
  // Insert modified string
  // document.getElementById("lettersAndDashes").innerHTML = newStr;
// GET ELEMENTS
...
document.onkeyup = function (event) {
  ...
  if (positions.length) {
    ...
    for (i = 0; i < positions.length; i++) {
      wordRandom[positions[i]] = lettersGuessed;
    }
